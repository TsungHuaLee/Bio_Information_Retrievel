...
class xmldata:
    def __init__(self, title=None, content=None, char_count=None, word_count=None, sentence_count=None, score=None, top_5_similarity = None):
        self.title = title
...
def tf_idf(title, content, total_document, key, type = 1):
    ...
    if(type == 1):
        ...
        idx_of_word = []
        for i in key:
            temp = np.where(word == i)
            idx_of_word.append(temp)
        ...
        for vec in X.toarray():
            ...
            for each_query_idx in idx_of_word:
                if(each_query_idx[0].size == 0):
                    each_query_term_count.append(0)
                else:
        ...
        # calculate idf
        inverse_docu_freq = [math.log10(total_document/(np.array(freq)+1e9)) for freq in docu_freq]
...
def mesh_full_text_match(file_name, key, tf_idf_type = 2):
    match_data = []
    match_ori_title = []
    match_ori_content = []
    ...
    print(len(match_ori_title), len(match_ori_content), total_document)
    tf_idf_result, top_5_similarity = tf_idf(match_ori_title, match_ori_content, total_document, key, tf_idf_type)
    ...
    for idx, a in enumerate(match_data):
        ...
        concat = ""
        for i in range(5):
            concat = concat + str(i) +". "+ top_5_similarity_title[idx][i] + " <br> "
        match_data[idx].similarity = concat
